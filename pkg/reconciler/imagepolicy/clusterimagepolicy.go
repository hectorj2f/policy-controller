...
import (
	...
	"github.com/sigstore/policy-controller/pkg/reconciler/imagepolicy/resources"
	webhookcip "github.com/sigstore/policy-controller/pkg/webhook/imagepolicy"
...
var _ imagepolicyreconciler.Interface = (*Reconciler)(nil)
var _ imagepolicyreconciler.Finalizer = (*Reconciler)(nil)

// ReconcileKind implements Interface.ReconcileKind.
func (r *Reconciler) ReconcileKind(ctx context.Context, ip *v1alpha1.ImagePolicy) reconciler.Event {
	ip.Status.InitializeConditions()
	ipCopy, ipErr := r.inlinePublicKeys(ctx, ip)
	if ipErr != nil {
		r.handleCIPError(ctx, ip.Name)
		// Update the status to reflect that we were unable to inline keys.
		ip.Status.MarkInlineKeysFailed(ipErr.Error())
		// Note that we return the error about the Invalid cip here to make
		// sure that it's surfaced.
		return ipErr
	}
	ip.Status.MarkInlineKeysOk()

	ipErr = r.inlinePolicies(ctx, ipCopy)
	if ipErr != nil {
		r.handleCIPError(ctx, ip.Name)
		// Update the status to reflect that we were unable to inline policies.
		ip.Status.MarkInlinePoliciesFailed(ipErr.Error())
		// Note that we return the error about the Invalid cip here to make
		// sure that it's surfaced.
		return ipErr
	}
	ip.Status.MarkInlinePoliciesOk()

	webhookCIP := webhookcip.ConvertImagePolicyV1alpha1ToWebhook(ipCopy)
	...
	if err != nil {
		if !apierrs.IsNotFound(err) {
			...
			ip.Status.MarkCMUpdateFailed(err.Error())
			return err
		}
		// Does not exist, create it.
		cm, err := resources.NewConfigMap(system.Namespace(), config.ImagePoliciesConfigName, ip.Name, webhookCIP)
		if err != nil {
		...
		ip.Status.MarkCMUpdatedOK()
		return err
	}

	// Check if we need to update the configmap or not.
	patchBytes, err := resources.CreatePatch(system.Namespace(), config.ImagePoliciesConfigName, ip.Name, existing.DeepCopy(), webhookCIP)
	if err != nil {
...
func (r *Reconciler) FinalizeKind(ctx context.Context, cip *v1alpha1.ImagePolicy) reconciler.Event {
...
func (r *Reconciler) inlinePublicKeys(ctx context.Context, cip *v1alpha1.ImagePolicy) (*v1alpha1.ImagePolicy, error) {
	ret := cip.DeepCopy()
...
func (r *Reconciler) inlineAndTrackSecret(ctx context.Context, cip *v1alpha1.ImagePolicy, keyref *v1alpha1.KeyRef) error {
...
func (r *Reconciler) inlinePolicies(ctx context.Context, cip *v1alpha1.ImagePolicy) error {
...
// is modified.
func (r *Reconciler) inlineAndTrackConfigMap(ctx context.Context, cip *v1alpha1.ImagePolicy, policyRef *v1alpha1.Policy) error {
